...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column("login_code", sa.String(length=6), nullable=True),
        sa.Column("login_code_expires_at", sa.DateTime(timezone=True), nullable=True),
        sa.Column("telegram_user_id", sa.BigInteger, nullable=True),
        sa.Column("telegram_chat_id", sa.BigInteger, nullable=True),
        sa.Column("telegram_auth_code", sa.String(length=10), nullable=True),
        sa.Column("telegram_code_expires_at", sa.DateTime(timezone=True), nullable=True),
    )
    op.create_index(
        "ix_master_telegram_auth_code",
        "master",
        ["telegram_auth_code"],
        unique=False,
    )
    op.create_unique_constraint("master_max_user_id_key", "master", ["max_user_id"])
    op.create_unique_constraint(
        "master_telegram_user_id_key", "master", ["telegram_user_id"]
    )
...
def downgrade() -> None:
    ...
    op.drop_index("ix_master_telegram_auth_code", table_name="master")
    op.drop_constraint("master_max_user_id_key", "master", type_="unique")
    op.drop_constraint("master_telegram_user_id_key", "master", type_="unique")
